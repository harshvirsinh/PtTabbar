...
open class PTCardTabBar: UIView {
    ...
    func add(item: UITabBarItem){
        ...
        self.addButton(with: item.image!, selectedImage: item.selectedImage!)
    }
    ...
    private func addButton(with image: UIImage, selectedImage: UIImage){
    ...
    func reloadViews(){
        ...
        for item in items {
            if let image = item.image, let selectedImage = item.selectedImage {
                addButton(with: image, selectedImage: selectedImage)
            } else {
                addButton(with: UIImage(), selectedImage: UIImage())
            }
        }
